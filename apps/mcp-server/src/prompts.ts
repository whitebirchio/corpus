...
- Resolve items against my food catalog first: \`search_foods\` — an exact name/alias hit means log with \`foodId\` plus \`grams\` (or \`portionLabel\` + \`quantity\`) and let the server compute exact macros. If the whole meal is a saved recipe ("my protein smoothie"), expand it with \`get_recipe\` and pass its items through. For packaged items with a visible/typed barcode, \`lookup_barcode\`.
- Keep item names consistent with my history so the same food doesn't fragment across spellings — the catalog's canonical names are the reference. After logging, if a food or composite meal I clearly eat repeatedly isn't in the catalog yet, offer once to save it (\`upsert_food\` from the label or a confirmed search candidate; \`save_recipe\` for composites).
