...
5. [Nutrition accuracy](05-nutrition-accuracy/SPEC.md) — accurate meal logging as Corpus replaces MacroFactor: a photo + caption estimation protocol (`log_meal_conversation` prompt), a personal food catalog + saved recipes with server-side portion→gram→macro math and USDA FoodData Central / Open Food Facts lookups (`search_foods`, `upsert_food`, `save_recipe`, `get_recipe`, `lookup_barcode`). Phases 1–2 built 2026-07-18 — see [implementation notes](05-nutrition-accuracy/IMPLEMENTATION.md) for the remaining manual steps (Neon migration, optional `FDC_API_KEY`). Phase 3 (PWA barcode-scanner UI, the PWA's first write surface) still to build.
