...
5. [Nutrition accuracy](05-nutrition-accuracy/SPEC.md) — accurate meal logging as Corpus replaces MacroFactor: a photo + caption estimation protocol (`log_meal_conversation` prompt), a personal food catalog + saved recipes with server-side portion→gram→macro math and USDA FoodData Central / Open Food Facts lookups (`search_foods`, `upsert_food`, `save_recipe`, `get_recipe`, `lookup_barcode`), and a PWA barcode scanner — the PWA's first write surface (Scan tab; zxing-wasm camera decoding, catalog → OFF → FDC resolution, logging through the same core `logMeal` path as chat). All three phases built 2026-07-18 — see [implementation notes](05-nutrition-accuracy/IMPLEMENTATION.md) for remaining manual steps.
